...
class FeedAdapter(oprtions: FirestoreRecyclerOptions<Post>, val context: Context): FirestoreRecyclerAdapter<Post, FeedAdapter.FeedViewHolder>(oprtions) {
    ...
    override fun onBindViewHolder(holder: FeedViewHolder, position: Int, model: Post) {
        ...
        val postDocument = firestore.collection("Posts").document(snapshots.getSnapshot(position).id)
